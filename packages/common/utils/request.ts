...
const fetchWithAbort = <T>(url: string, options: FetchOptions = {}): FetchWithAbortType<T> => {
        ...
        window.fetch(url, opts)
            ...
            .catch((error) => {
                if (error.name === 'AbortError') {
                    ...
                    publishRequestError({ url, code: (error as any).status, message: error.message }, 'http');
                    reject(error);
                }
            })
...
export const fetch = <T>(url: string, options: FetchOptions = {}): Promise<T> => {
