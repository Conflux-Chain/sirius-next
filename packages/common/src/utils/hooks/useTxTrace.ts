...
import type { AbiParameter, AbiStateMutability } from 'src/utils/sdk';
...
export interface AbiFunctionWithoutGas {
  type: 'function';
...
const formatTraceData = (data: OriginTraceData) => {
  ...
  const formatItem = (
    ...
    const methodABI =
      methodId && t.action.to ? methodMap[methodId]?.[t.action.to] : undefined;
    if (methodABI) {
      try {
    ...
    const item: TreeTraceForUI = {
      ...
      fromContractInfo: contractInfo[t.action.from] || {},
      toContractInfo: contractInfo[t.action.to ?? ''] || {},
      fromTokenInfo: tokenInfo[t.action.from] || {},
      toTokenInfo: tokenInfo[t.action.to ?? ''] || {},
      isCallImpl: t.isCallImpl,
    };
    if (t.action.fromSpace === 'evm') {
      item.fromESpaceInfo = {
        address: formatAddress(t.action.from, 'hex'),
      };
    }
    if (t.action.toSpace === 'evm') {
      item.toESpaceInfo = {
        address: formatAddress(t.action.to, 'hex'),
      };
    }
    if (
      parent &&
...
export const useTxTrace = (hash: string) => {
  ...
  return useSWRImmutable(url, () =>
    fetchWithPrefix(url)
      .then((resp: any) => {
        if (resp?.traceTree) {
          const { list, total } = formatTraceData(resp);
          return { list, total };
        }
      })
      .catch(e => {
...
export const useCFXTransferListInTx = (hash: string) => {
  const a = useTxTrace(hash);
};
