...
describe('roundToFixedPrecision', () => {
  ...
  test('preserves non-numeric suffix in strings', () => {
    expect(roundToFixedPrecision('123K', 2)).toBe('123.00K');
  });

  // Test string with prefix and suffix
  test('handles strings with both prefix and suffix', () => {
    expect(roundToFixedPrecision('+123abc', 2)).toBe('123.00abc');
  });

  // Test incorrect rounding method
  test('defaults to ROUND method when an incorrect method is provided', () => {
    expect(roundToFixedPrecision(1.005, 2, 'INVALID')).toBe('1.01');
  });

  // Test empty string
  test('returns zero with specified precision for an empty string', () => {
    expect(roundToFixedPrecision('', 2)).toBe('--');
  });

  // Test non-numeric string
  test('returns NaN when the string is non-numeric', () => {
